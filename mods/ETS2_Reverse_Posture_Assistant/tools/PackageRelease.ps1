...
foreach ($variant in @('Full', 'Runtime_for_Workshop', 'Upgrade_from_v0.6.0')) {
    ...
    Copy-Item -LiteralPath "$root/README.md","$root/RELEASE_NOTES_v0.11.0.md","$root/LICENSE","$root/THIRD_PARTY_NOTICES.md" -Destination $stage
    ...
    if ($variant -eq 'Runtime_for_Workshop') {
        ...
        Copy-Item -LiteralPath "$release/ETS2_Reverse_Posture_Assistant_v0.11.0.scs" -Destination "$stage/mod/ETS2_Reverse_Posture_Assistant_1.61.scs"
    ...
    $archive = Join-Path $release "ETS2_Reverse_Posture_Assistant_v0.11.0_$variant.zip"
    ...
    try {
        ...
        if ($variant -ne 'Runtime_for_Workshop' -and -not $zip.GetEntry('mod/ETS2_Reverse_Posture_Assistant_1.61.scs')) { throw 'Packaged mod missing' }
